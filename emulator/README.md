# CrossPoint Reader — Web Emulator

A browser-based emulator for the CrossPoint Reader firmware, compiled to WebAssembly via Emscripten.

## How It Works

The emulator compiles the **exact same firmware code** that runs on the XTeink X4 hardware to WebAssembly. Instead of talking to real hardware (e-ink display, buttons, SD card), the firmware talks to browser-based replacements:

| Firmware HAL | Hardware (X4)        | Emulator (Browser)           |
| ------------ | -------------------- | ---------------------------- |
...
This produces files in `emulator/build/`:
...
3. **Upload an EPUB**: Use the file picker to load a `.epub` file into the emulator's virtual filesystem.
...
├── main_emulator.cpp       # Entry point (replaces Arduino main)
...
**Key design principle**: Zero firmware modifications. All emulator code lives in `emulator/`. The firmware source files compile unmodified — our shim headers are found first via include path ordering.
...
**Display doesn't update**: The emulator renders the framebuffer to canvas whenever `displayBuffer()` is called by the firmware. If nothing appears, the firmware may not have reached a render call yet.
