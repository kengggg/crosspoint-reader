...
// in the emulator (deep ESP32/ArduinoJson dependencies).
// These classes are non-functional in the web emulator.
...
void CrossPointWebServer::begin() {
  printf("[EMU] CrossPointWebServer::begin() - not available in emulator\n");
}
...
bool HttpDownloader::fetchUrl(const std::string& url, std::string& outContent) {
  (void)url;
  outContent.clear();
  printf("[EMU] HttpDownloader::fetchUrl() - not available in emulator\n");
  return false;
}
...
bool HttpDownloader::fetchUrl(const std::string& url, Stream& stream) {
  (void)url;
  (void)stream;
  printf("[EMU] HttpDownloader::fetchUrl(stream) - not available in emulator\n");
  return false;
}
...
HttpDownloader::DownloadError HttpDownloader::downloadToFile(
  ...
  (void)destPath;
  (void)progress;
  printf("[EMU] HttpDownloader::downloadToFile() - not available in emulator\n");
  return HTTP_ERROR;
}
...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  printf("[EMU] OtaUpdater::checkForUpdate() - not available in emulator\n");
...
OtaUpdater::OtaUpdaterError OtaUpdater::installUpdate() {
  printf("[EMU] OtaUpdater::installUpdate() - not available in emulator\n");
  return INTERNAL_UPDATE_ERROR;
}
...
KOReaderSyncClient::Error KOReaderSyncClient::authenticate() {
  printf("[EMU] KOReaderSyncClient::authenticate() - not available in emulator\n");
...
const char* KOReaderSyncClient::errorString(Error error) {
  switch (error) {
    ...
    case NETWORK_ERROR: return "Network not available (emulator)";
