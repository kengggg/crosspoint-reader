// HalGPIO implementation for Emscripten web emulator.
...
extern "C" {
  ...
  void emulator_button_down(int buttonIndex) {
  ...
  EMSCRIPTEN_KEEPALIVE
  void emulator_button_up(int buttonIndex) {
    if (buttonIndex >= 0 && buttonIndex < 7) {
...
void HalGPIO::begin() {
  ...
  printf("[EMU] HalGPIO initialized (keyboard input)\n");
}
...
void HalGPIO::startDeepSleep() {
  printf("[EMU] Deep sleep requested (ignored in web emulator)\n");
  // In the web emulator, "deep sleep" just clears the screen
  EM_ASM({
...
int HalGPIO::getBatteryPercentage() const {
  return 100;  // Always full battery in emulator
}
