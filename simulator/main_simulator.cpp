// Emscripten entry point for CrossPoint Reader web simulator.
//
...
extern void simulator_fiber_init();
extern void simulator_run_background_tasks(int elapsed_ms);
...
static void em_main_loop() {
  ...
  simulator_run_background_tasks(elapsed_ms);
}

int main() {
  printf("[SIM] CrossPoint Reader Web Simulator starting...\n");

  // Initialize the fiber system for cooperative multitasking
  simulator_fiber_init();

  // Run the firmware's setup() function
  printf("[SIM] Running firmware setup()...\n");
  setup();
  printf("[SIM] Setup complete, starting main loop.\n");
