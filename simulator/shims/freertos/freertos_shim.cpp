// FreeRTOS cooperative task scheduler for Emscripten web simulator.
//
...
BaseType_t xTaskCreate(TaskFunction_t pvTaskCode, const char* pcName, uint32_t usStackDepth, void* pvParameters,
  ...
  if (g_task_count >= MAX_TASKS) {
    printf("[SIM] ERROR: Max tasks (%d) exceeded!\n", MAX_TASKS);
    return pdFAIL;
  }

  int idx = g_task_count;
  SimulatorTask& task = g_tasks[idx];
  ...
  g_task_count++;
  printf("[SIM] Task created: '%s' (index %d)\n", task.name, idx);
  return pdPASS;
}
...
void vTaskDelete(TaskHandle_t xTaskToDelete) {
  ...
  if (idx >= 0 && idx < g_task_count) {
    ...
    printf("[SIM] Task deleted: '%s' (index %d)\n", g_tasks[idx].name, idx);
